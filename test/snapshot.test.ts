...
test('writeSnapshot emits shards and a schema-correct manifest', async () => {
  ...
  const manifest = await writeSnapshot(dir, {
    ...
    users: [{ did: 'did:key:z6MkA', alias: null, maintains: [repo.rid], issues: 0, patches: 0 }],
    knownRepos: 6528,
    signer: SIGNER,
    indexerName: 'test',
    now: 42,
...
test('signManifest produces a verifiable EIP-191 signature', async () => {
  ...
  assert.equal(verifyMessage(canonicalManifestForSigning(signed), signed.sig), SIGNER);
